...
const App = () => {
  ...
  // ============================================================================
  // WI-FI AND POSITIONING FUNCTIONS (Enhanced with task progress tracking)
  // ============================================================================
  ...
  // ============================================================================
  // TRACKING FUNCTIONS (Enhanced with task management)
  // ============================================================================
          ...
          <View style={styles.locationVerification}>
            ...
            <Text style={[styles.verificationStatus, { 
              ...
              {currentArea === task.area_label ? 
                ...
                `⚠️ Expected: ${task.area_label}, Detected: ${currentArea}`
              }
            </Text>
          </View>
        )}
  ...
  const renderCleanerMode = () => (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <Text style={styles.title}>Cleaner Mode</Text>
        <Text style={styles.subtitle}>Task management and position tracking</Text>
      </View>
      ...
      {/* Tracking Control */}
      <View style={styles.card}>
        <Text style={styles.cardTitle}>🛰️ Tracking Control</Text>
        <View style={styles.trackingContainer}>
          <View style={styles.trackingInfo}>
            <Text style={styles.trackingText}>
              {isTracking ? '✅ Tracking Active' : '⏸️ Tracking Paused'}
            ...
            <Text style={styles.trackingSubtext}>
              {isTracking ? 
                `Updates every ${batteryOptimized ? '15' : '8'} seconds` : 
                activeTask ? 'Automatic tracking when task is active' : 'Tap to start tracking'
              }
            </Text>
          </View>
          {!activeTask && (
        ...
        {activeTask && (
          <Text style={styles.autoTrackingNote}>
            🤖 Automatic tracking is enabled for your active task
          </Text>
        )}
      </View>
  ...
  if (loading && (!tasks || tasks.length === 0) && user?.role === 'cleaner') {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color="#007AFF" />
        <Text style={styles.loadingText}>Initializing Indoor Positioning System...</Text>
        <Text style={styles.loadingSubtext}>Loading your tasks and setting up sensors</Text>
      </View>
    );
  }
